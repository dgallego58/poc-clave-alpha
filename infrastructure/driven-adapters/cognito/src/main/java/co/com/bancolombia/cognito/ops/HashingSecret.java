...
public class HashingSecret {
    ...
    public String hashing(String username) {
        var instance = CognitoBaseProperties.getInstance();
        var signingKey = new SecretKeySpec(instance.getAppClientSecret().getBytes(StandardCharsets.UTF_8),
                HMAC_SHA256_ALGORITHM);
        try {
            Mac mac = Mac.getInstance(HMAC_SHA256_ALGORITHM);
            mac.init(signingKey);
            mac.update(username.getBytes(StandardCharsets.UTF_8));
            byte[] rawHmac = mac.doFinal(instance.getAppClientId().getBytes(StandardCharsets.UTF_8));
